...
public class LootTableAPI {
    ...
    /**
     * Usuwa WSZYSTKIE modyfikacje dla określonego loot table
     * WAŻNE: To czyści modyfikacje tylko z pamięci API, nie z configów!
     *
     * @param tableId Identifier loot table do wyczyszczenia
     */
    public static void clearModifications(Identifier tableId) {
        if (modifiers.remove(tableId) != null) {
            LootLogger.logInfo("Cleared all modifications for: " + tableId);
            totalTablesModified--;
        }

        if (multipliers.remove(tableId) != null) {
            LootLogger.logInfo("Removed multiplier for: " + tableId);
        }
    }

    /**
     * Usuwa WSZYSTKIE modyfikacje ze WSZYSTKICH loot tables
     * OSTRZEŻENIE: To czyści całą pamięć API!
     * Używaj tylko gdy chcesz zresetować wszystkie zmiany
     */
    public static void clearAllModifications() {
        int tablesCleared = modifiers.size();
        int multipliersCleared = multipliers.size();
        ...
        LootLogger.logInfo("Tables cleared: " + tablesCleared);
        LootLogger.logInfo("Multipliers cleared: " + multipliersCleared);
    }

    /**
     * Sprawdza czy loot table ma jakiekolwiek modyfikacje
     *
     * @param tableId Identifier loot table
     * @return true jeśli ma modyfikacje
     */
    public static boolean hasModifications(Identifier tableId) {
        return modifiers.containsKey(tableId) || multipliers.containsKey(tableId);
    }

    /**
     * Zwraca listę wszystkich zmodyfikowanych loot tables
     *
     * @return Set identifierów
     */
    public static Set<Identifier> getModifiedTables() {
        Set<Identifier> allModified = new HashSet<>();
        allModified.addAll(modifiers.keySet());
        allModified.addAll(multipliers.keySet());
        return allModified;
    }
}